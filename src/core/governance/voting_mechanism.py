...
if __name__ == "__main__":
    ...
    voting_mechanism = VotingMechanism(provider_url, private_key)

    # Create proposals
    voting_mechanism.create_proposal("Increase the budget for marketing.")
    voting_mechanism.create_proposal("Launch a new product line.")

    # Simulate voting
    voting_mechanism.vote(0, vote_for=True)  # Vote for the first proposal
    voting_mechanism.vote(0, vote_for=False)  # Vote against the first proposal
    voting_mechanism.vote(1, vote_for=True)   # Vote for the second proposal

    # Execute proposals
    voting_mechanism.execute_proposal(0)
    voting_mechanism.execute_proposal(1)
